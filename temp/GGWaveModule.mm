...
- (instancetype)init {
    ...
    if (self) {
        ...
        audioInputState.moduleRef = self;
        audioOutputState.moduleRef = self;
...
- (void)handleAudioSessionInterruption:(NSNotification *)notification {
    ...
    switch (interruptionType) {
        case AVAudioSessionInterruptionTypeBegan:
            RCTLogInfo(@"Audio session interruption began");
            if (_isListening) {
            ...
            if (_isTransmitting) {
                ...
                [self sendEventWithName:@"onTransmissionCompleted" body:@{@"success": @NO, @"error": @"Interrupted"}];
            }
            break;
            
        case AVAudioSessionInterruptionTypeEnded: {
            RCTLogInfo(@"Audio session interruption ended");
            ...
            if (options & AVAudioSessionInterruptionOptionShouldResume) {
                // Could automatically resume here if needed
                RCTLogInfo(@"Audio session can resume");
            }
            break;
        }
    }
...
- (void)handleAudioSessionRouteChange:(NSNotification *)notification {
    ...
    switch (reason) {
        ...
        case AVAudioSessionRouteChangeReasonOldDeviceUnavailable:
            RCTLogInfo(@"Audio route changed: %ld", (long)reason);
...
- (void)handleAppDidEnterBackground:(NSNotification *)notification {
    RCTLogInfo(@"App entering background - pausing audio operations");
    ...
    if (_isTransmitting) {
        [self stopAudioPlayback];
        [self sendEventWithName:@"onTransmissionCompleted" body:@{@"success": @NO, @"error": @"Backgrounded"}];
    }
}

- (void)handleAppWillEnterForeground:(NSNotification *)notification {
    RCTLogInfo(@"App entering foreground - audio operations can resume");
}
...
- (BOOL)initializeGGWave {
    ...
    // RX (Receive) instance
    {
        ...
        if (ggwaveRxInstance < 0) {
            RCTLogError(@"Failed to initialize ggwave RX instance");
            return NO;
        }
        
        RCTLogInfo(@"GGWave RX instance initialized - id: %d", ggwaveRxInstance);
    }
    ...
    // TX (Transmit) instance
    {
        ...
        if (ggwaveTxInstance < 0) {
            RCTLogError(@"Failed to initialize ggwave TX instance");
            return NO;
        }
        
        RCTLogInfo(@"GGWave TX instance initialized - id: %d", ggwaveTxInstance);
    }
...
- (BOOL)startAudioListening {
    RCTLogInfo(@"Starting audio listening");
    
    if (audioInputState.isCapturing) {
        RCTLogInfo(@"Already capturing audio");
        return YES;
    }
    ...
    if (permission == AVAudioSessionRecordPermissionDenied) {
        RCTLogError(@"Microphone permission denied");
        [self sendEventWithName:@"onError" body:@{@"error": @"Microphone permission denied"}];
        return NO;
    }
    ...
    if (permission == AVAudioSessionRecordPermissionUndetermined) {
        ...
        [audioSession requestRecordPermission:^(BOOL granted) {
            if (granted) {
                ...
                RCTLogError(@"Microphone permission denied by user");
                [self sendEventWithName:@"onError" body:@{@"error": @"Microphone permission denied"}];
            }
        }];
        return NO;
    ...
    if (status != noErr) {
        RCTLogError(@"Failed to create audio input queue: %d", (int)status);
        return NO;
    }
    ...
    RCTLogInfo(@"Audio listening started successfully");
    return YES;
}

- (BOOL)stopAudioListening {
    RCTLogInfo(@"Stopping audio listening");
    
    if (!audioInputState.isCapturing) {
        RCTLogInfo(@"Not currently capturing audio");
        return YES;
    }
    ...
    RCTLogInfo(@"Audio listening stopped successfully");
    return YES;
}
...
- (BOOL)transmitAudioMessage:(NSString *)message {
    ...
    if (ggwaveTxInstance < 0) {
        RCTLogError(@"TX instance not initialized");
        return NO;
    }
    
    if (audioOutputState.isPlaying) {
        RCTLogError(@"Already playing audio");
        return NO;
    }
    ...
    if (waveformSize <= 0) {
        RCTLogError(@"Failed to query waveform size: %d", waveformSize);
        return NO;
    }
    ...
    if (actualSize <= 0) {
        RCTLogError(@"Failed to encode message: %d", actualSize);
        return NO;
    }
    ...
    if (status != noErr) {
        RCTLogError(@"Failed to create audio output queue: %d", (int)status);
        return NO;
    }
    ...
    if (status != noErr) {
        RCTLogError(@"Failed to start audio output queue: %d", (int)status);
        [self stopAudioPlayback];
        return NO;
    }
    
    RCTLogInfo(@"Audio transmission started successfully");
    return YES;
}

- (void)stopAudioPlayback {
    RCTLogInfo(@"Stopping audio playback");
    ...
    RCTLogInfo(@"Audio playback stopped successfully");
}
                        ...
                        const AudioStreamPacketDescription * inPacketDescs)
{
    ...
    if (ret > 0) {
        ...
        // Send React Native event for received message
        GGWaveModule * module = state->moduleRef;
        if (module) {
            ...
            [module sendEventWithName:@"onMessageReceived" body:eventData];
            RCTLogInfo(@"Received message: %@", receivedMessage);
        }
    }
                         ...
                         AudioQueueBufferRef outBuffer)
{
    ...
    if (nRemainingBytes > 0) {
        ...
        if (status != noErr) {
            RCTLogError(@"Failed to enqueue audio data: %d", (int)status);
        }
        ...
        if (state->isPlaying) {
            ...
            // Send React Native event for transmission completed
            GGWaveModule * module = state->moduleRef;
            if (module) {
                [module sendEventWithName:@"onTransmissionCompleted" body:@{@"success": @YES}];
            }
        }
