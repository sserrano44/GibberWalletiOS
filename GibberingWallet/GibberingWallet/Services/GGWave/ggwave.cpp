...
bool GGWave::alloc(void * p, int & n) {
    ...
    // pre-allocate Reed-Solomon memory buffers
    {
        ...
        if (m_isFixedPayloadLength == false) {
            ::ggalloc(m_workRSLength, static_cast<int>(RS::ReedSolomon::getWorkSize_bytes(1, m_encodedDataOffset - 1)), p, n);
        }
        ::ggalloc(m_workRSData, static_cast<int>(RS::ReedSolomon::getWorkSize_bytes(maxLength, getECCBytesForLength(maxLength))), p, n);
    }
...
bool GGWave::init(const char * text, TxProtocolId protocolId, const int volume) {
    return init(static_cast<int>(strlen(text)), text, protocolId, volume);
}
